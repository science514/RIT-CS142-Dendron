...
public class DendronTest {
    ...
    private static List<List<String>> programs = Arrays.asList(new LinkedList<>(Arrays.asList(":=", "x", "55")),
            new LinkedList<>(
                    Arrays.asList(":=", "able", "77", ":=", "baker", "3", ":=", "charlie", "/", "able", "baker")),
            new LinkedList<>(Arrays.asList(":=", "a", "3", ":=", "b", "4", ":=", "c", "5", ":=", "result", "+", "*",
                    "b", "b", "_", "*", "*", "4", "a", "c")),
            new LinkedList<>(Arrays.asList(":=", "x", "1", ":=", "x", "+", "x", "x", ":=", "x", "*", "x", "x", ":=",
                    "x", "-", "2", "_", "x", "@", "x", ":=", "x", "/", "x", "2", ":=", "pastafagiole", "#", "+", "19",
                    "x")),
            new LinkedList<>(Arrays.asList(":=", "a", "1", ":=", "b", "_", "1", ":=", "c", "_", "6", ":=", "root", "/",
                    "+", "_", "b", "#", "-", "*", "b", "b", "*", "*", "4", "a", "c", "*", "2", "a", ":=", "root2", "/",
                    "-", "_", "b", "#", "-", "*", "b", "b", "*", "*", "4", "a", "c", "*", "2", "a")),
            ...
            new LinkedList<>(Arrays.asList(":=", "x", "9", ":=")), new LinkedList<>(Arrays.asList(":=", "y")),
     ...
     *
     * @param args if numeric and the number is less than the number of private
     *             stored tests, run the private test corresponding to that number;
     *             if other args, consider them tokens of a Dendron program and run
     *             tests on that program; if no arguments, read the source program
     *             from standard input.
     */
    public static void main(String... args) {
        ...
        if (args.length == 0) {
            ...
            try {
                ...
                if (files == null) {
                    System.err.println("Provided directory " + args[0] + " does not exist.");
                    System.exit(1);
                } else {
                    for (File file : files) {
